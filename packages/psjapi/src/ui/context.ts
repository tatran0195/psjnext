...
export interface CreatePSJAPIPageOptions {
   ...
   *
   * Defaults to "/docs/<normalized-ref>".
   * Override to match your actual baseUrl:
   *
   *   resolveRef: (ref) => `/sdk/${ref.toLowerCase().replace(/\/g, '-')}`
   */
...
export interface ResolvedPSJAPIPageOptions
  extends Omit<CreatePSJAPIPageOptions, 'shiki' | 'shikiOptions'> {
  shiki: ShikiFactory;
  shikiOptions: Omit<CodeToHastOptionsCommon, 'lang'> & CodeOptionsThemes<BundledTheme>;
}
