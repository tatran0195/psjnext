...
function CalloutBox({ level, children }: { level: CalloutLevel; children: ReactNode }) {
  return (
    <div className={cn('not-prose flex gap-3 rounded-lg border p-4 text-sm my-3', CALLOUT_STYLES[level])}>
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center gap-3 not-prose">
        <span className={cn('font-medium font-mono text-fd-primary', removed && 'line-through')}>
          {name}
          {required ? (
