...
function ParamRow({ param, isMacro }: { param: ResolvedParam; isMacro: boolean }) {
    return (
        <div
            className="grid gap-x-4 border-b border-fd-border py-3 last:border-0 text-sm"
        ...
        >
            ...
            <div className="min-w-0">
                <div className="flex items-baseline gap-2 flex-wrap">
                    ...
                    {param.deprecated && (
                        <span className="text-xs text-fd-muted-foreground line-through">
                            deprecated
                        </span>
                    )}
                </div>
...
function ParamsSection({ params, isMacro }: { params: ResolvedParam[]; isMacro: boolean }) {
...
function DefaultItemLayout({ slots }: { slots: ItemLayoutSlots }) {
    return (
        <div className="grid @container grid-cols-1 @3xl:grid-cols-[1fr_min(40%,28rem)] gap-8 items-start text-sm">
            ...
            {/* RIGHT: examples */}
            <div className="flex flex-col gap-4 @3xl:sticky @3xl:top-20">{slots.examplesPanel}</div>
        </div>
    );
}
...
export async function PSJAPIItemRenderer({
    ...
    function renderHeading(
        ...
        if (options.renderHeading)
            return options.renderHeading({ children: text, ...props }, depth);
        return (
    ...
    const calloutTexts = await Promise.all(item.callouts.map((c) => renderMarkdown(c.text)));
    const calloutsSlot =
    ...
    const returnsSlot = (
        <>
            ...
            {renderReturns ? renderReturns(item, ctx) : <ReturnsSection returns={item.returns} />}
        </>
    );
