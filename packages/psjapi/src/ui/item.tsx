...
function TypeWithRefs({ type, resolveRef, locale }: { type: string; resolveRef: (ref: string, locale: string) => string; locale: string }) {
...
function ParamProperty({
    ...
    const typeNode = <TypeWithRefs type={param.type} resolveRef={resolveRef} locale={locale} />;
...
function ReturnsSection({
    ...
    if (returns.kind === 'typed') {
        return (
            <Property
                name="return"
                type={<TypeWithRefs type={returns.type ?? 'unknown'} resolveRef={resolveRef} locale={locale} />}
                required={false}
            >
                {returns.description && (
...
function SeeAlsoSection({
    ...
    return (
        <div className="not-prose flex flex-col gap-1.5">
            {refs.map((ref) => {
                const href = resolveRef(ref.$ref, locale);
...
export async function PSJAPIItemRenderer({
    ...
    function resolveRef(ref: string): string {
        if (resolveRefOption) return resolveRefOption(ref, locale);
        // Default: /docs/<locale>/<normalized-ref>
        ...
        item.params.length > 0 ? (
            <>
                ...
                <div className="flex flex-col">
                    {renderParam
                        ? item.params.map((p, i) => (
                            <Fragment key={i}>{renderParam(p, ctx)}</Fragment>
                        ))
                        : item.params.map((p, i) => (
                            <ParamProperty
    ...
    const returnsSlot = (
        <>
            ...
            {renderReturns ? (
                ...
                <div className="flex flex-col">
                    <ReturnsSection returns={item.returns} resolveRef={resolveRef} locale={locale} />
                </div>
            )}
        </>
    );
    ...
    const seeAlsoSlot =
        item.see_also.length > 0 ? (
            <>
                ...
                <SeeAlsoSection refs={item.see_also} resolveRef={resolveRef} locale={locale} />
            </>
        ) : null;
