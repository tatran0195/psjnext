...
import type { PsjPagesBuilderConfig, OutputEntry, ItemOutput, PageOutput } from './utils/pages/builder';
...
function generateMetaFiles(
  ...
  const { groupStyle = 'folder' } =
    typeof options.meta === 'object' ? options.meta : {};
  const pages: string[] = [];
