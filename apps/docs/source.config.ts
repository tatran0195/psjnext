...
const psjPath = [
    path.resolve(import.meta.dirname, 'psj.json'),
    path.resolve(import.meta.dirname, '../psj.json'),
    path.resolve(process.cwd(), 'apps/docs/psj.json'),
    path.resolve(process.cwd(), 'psj.json'),
].find((p) => fs.existsSync(p));

if (!psjPath) {
    throw new Error('Could not find psj.json');
}

const psjGrammar = {
    ...JSON.parse(fs.readFileSync(psjPath, 'utf8')),
    id: 'psj',
    name: 'psj',
};
