...
# PSJ GUI EXAMPLES
# ---------------------
# This example demonstrades how to create a simple part with PSJ by using PSJ-Command
#*******************************************************************************
...
def on_execute_button_clicked(dlg, part_name, coord_system, bolt_type, bolt_length):
    ...
    Geometry.Part.Cylinder(strName=part_name,
                           ...
                           dTopRadius=nut_radius,
                           dBottomRadius=nut_radius,
                           dHeight=cylinder_height,
                           iCircularNodes=6,
                           iPartColor=color)
    Geometry.Part.Cylinder(strName=part_name,
                           crLocalCoordinate=Coord(coord_system),
                           dlOrigin=[0.000, cylinder_position, 0.000],
                           dTopRadius=thread_radius,
                           dBottomRadius=thread_radius,
                           dHeight=get_param_bolt_length,
                           iPartColor=color)

    # Making assemble faces
    all_part = sorted([a.id for a in JPT.GetAllParts()], reverse = False)
    assemble_faces = Assemble.AssembleFaceEx(ilPairFaceToMakeShareFace=Assemble.FindMatingFaceEx(crlTaBodies=[Part(int(all_part[-2]),
                                                                                                                   int(all_part[-1]))],
                                                                                                 dMatingTol=1e-05),
                                             dTolerance=1e-05, iTypeConnectPos=0)

    # Merging 2 created parts into 1 part
    Geometry.MergeEntities.Parts(dMergeTolerance=1e-05, crlParts=[Part(all_part[-2], all_part[-1])])
    ...
    if str(dlg.get_item_text("part_name")).find("_") != -1:
        if dlg.get_item_text("part_name").split("_")[-1].isdigit() == True:
            new_name = dlg.get_item_text("part_name")[:-(len(dlg.get_item_text("part_name").split("_")[-1]) + 1)] \
                       + "_" + str(int(dlg.get_item_text("part_name").split("_")[-1]) + 1)
        else:
