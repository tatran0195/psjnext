...
### `iProcess` @type(Integer) @default(0)
...
### `iWrappingType` @type(Integer) @default(0)

- The wrapping type. This argument only was used when`iProcess`= 1 or 2.
  - 0: Tight
  - 1: Convex

### `dlSphereCenter` @type(List\[Double]) @default(\[])

- The center coordinates of sphere.

### `dSphereRadius` @type(Double) @default(0.0)

- The radius of sphere.

### `crTargetPart` @type(Cursor) @default(None)
