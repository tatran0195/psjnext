...
- The CAD Parts. CAD Faces in these parts will be the reference faces for projection.
  - Note that for CAD part,_Curso&#x72;_&#x74;ype will take different format from normal cursor's format. Two parameters are now needed: the former is ID (external id) and the latter is Key (internal id).
  - For example: RefPart((1,2)) means a CAD part with ID (external id) = 1 and Key (internal id) = 2.

### `crMeshedPart` @type(Cursor) @required
