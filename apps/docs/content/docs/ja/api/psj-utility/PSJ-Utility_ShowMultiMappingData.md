...
- Specify controls the data display type when Convection Mapping is selected.
...
def setup _mapping _view():
    ...
    JPT.ViewFitToModel()
    conv _maps = JPT.GetAllByTypeID(JPT.EntityType.LBC _MAPPING _THERMAL _CONVECTION)
    JPT.ShowMultiMappingData(conv _maps)
