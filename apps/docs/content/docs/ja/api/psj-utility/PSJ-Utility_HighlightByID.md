...
- Specify _[DItemType](../data-type/psj-command/DItem-types)_ of the target entities.

<!-- @since:5.1.0 @required -->
### entityID

- Specify ID of the selecting entity.
...
BoundaryConditions.Force.General(forceLBC=FORCE _LBC(vecForce=[DFLT _DBL, DFLT _DBL, -10.0]), 
    crlTargets=[Face(52)])
...
for lbc in listDItemLoadBCs:
    if lbc.type == JPT.DItemType.LBC _G_PRESSURE:
        iID = lbc.id
JPT.HighlightByID(JPT.DItemType.LBC _G_PRESSURE, iID, JPT.BoolType.TRUE _VAL)
```
