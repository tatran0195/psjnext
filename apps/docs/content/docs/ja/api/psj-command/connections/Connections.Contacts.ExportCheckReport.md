...
- Specify the destination path of the HTML/Excel file to be exported. The destination path should be different from the C Drive (C:/) due to Window would deny saving any files to the C Drive directly (It is recommended to save in User's Drive such as D Drive, E Drive,...).

<!-- @since:5.0.1 @optional -->
### dZoomFactor

- Specify the zoom factor when capturing the image.
...
- Specify which entity type will be focused on when capturing the image.
...
- Specify the contact report will be listed by type.
...
- Specify the order sort type of contacts in report.
