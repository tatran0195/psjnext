...
- Specify the list of parts to measure the mass. This argurment could be in Part type and/or Bar type. If this parameter is \[],`crlMassConditions` must be specified.
...
- Specify the list of mass conditions created by_[Connections.MassElements](../connections/Connections.MassElements)_. If this is \[], `crlParts` must be specified.
...
- Specify the material density (default unit: kg/m3).

<!-- @since:5.0.1 @optional -->
### strTarget

- Specify the target value of mass information to be outputted. Depending on the target information, this parameter can be one of the following:
...
- Specify the use of gravity center Coordinate System to measure the mass.
...
- Specify the Coordinate System to measure the mass. This parameter cannot be used when`bGravityCenter` is _True_.
- The default value is _None_ (Global Coordinate).

<!-- @since:5.0.1 @optional -->
### iPrecision

- Specify the number of digit after floating point. The greater`iPrecision` could be, the more accuracy of mass value can be measured.
