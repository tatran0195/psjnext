...
<Callout type="info" title="In this tutorial, you’ll learn">
  1. How to get and make use of the PSJ-Command/Jupiter's Macro. 2. Implement the PSJ-Command/Jupiter's Macro to a
     ...
     user script in (2.) to the created dialog in (3.) to make a custom UI function. 5. Add the created custom UI function
     in (4.) to Jupiter's ribbon and use it.
</Callout>
...
1. **Create DEF function in Python**: Combine PSJ-Command or Macro with Python based script in order to create an
   user-defined function fully.
1. **Create User Interface (UI)**: Use GUI Command Builder to make an UI dialog connecting to the user-defined function
   in (2.).
1. **Implement**: Register the created UI dialog in (3.) to Jupiter.
...
<Tabs groupId="commands-macros" defaultIndex={0} values={['psj', 'macro']}>
  <Tab value="psj">
    ```psj
...
1. Click on the Find button to find all the faces between the created 2 cylinders.
1. Click on the Apply or OK button to generate share faces finding from (2.).
...
1. Click on the Apply or OK button to merge them into 1 part.
...
<Callout type="info" title="To execute the code in Python, use 2 ways as below:">
  1. PSJ-Command: Copy and paste directly to the creating Python code. 2. Macro: use the following structure to execute
     the code in python`JPT.Exec(“Macro”)`.
</Callout>

<Callout type="info">
  The PSJ-Command and/or Macro is added to create the bolt according to the value generated by user. From now, user can
  combine the knowledge about Python language and the just generated Macro to create def function.
</Callout>
...
1. **listdir**, **isfile**, **join** are the libraries helping you to get all files inside a specific folder.
1. **pyjdg** is a library to enable GUI Command Builder.
1. **random** is a library to get a random number (for setting part's color).
...
**picture\_folder** is a parameter to get full path to the stored preview pictures.

> We stored all the related files relating to this tutorial in the Jupiter installation folder.
>
> **picture\_folder** is the parameter refering to the stored location to get stored pictures. The stored localtion:
>
> <menuselection>Jupiter installation folder » SampleData » PSJ » PSJ-GUI » CreateBolt » CreatingBolt\_Pics </menuselection>
...
<Callout type="info">
  Whenever start using a function, you should check it's output values and type. In Jupiter, we're supporting a utility
  called [JPT.Debugger](/docs/psj-utility/JPT.Debugger)
</Callout>
...
Based on the created PSJ-Command and Jupiter's Macro from the previous steps, the **on\_execute\_button\_clicked()**
function is responsible for implementing the Apply or OK button. It will have the following structure:
...
1. After clicking on the Dialog component (1.), change its properties as the below suggestions:
   <br />
...
1. Add _an image_ as a preview picture into the creating dialog by using **ImageCtrl** component.
1. Add _Part Name_ textbox to input the name of the creating bolt by using **Layout**, **Label** and **TextBox**
   components.
1. Add _Coordinate System_ selection to select the reference coordinate system for creating bolt by using **Layout**,
   **Label** and **ComboBox** components.
1. Add _Bolt Type_ selection to select the reference coordinate system for creating bolt by using **Layout**, **Label**
   and **ComboBox** components.
1. Add _Update preview bolt size_ button to update the preview bolt picture on the GUI based on the selected bolt type
   by using **Layout** and **Button** components.
1. Add _Bolt Length_ textbox to input the length of the creating bolt by using **Layout**, **Label** and **TextBox**
   components.
...
1. Click on the <kbd>Export PSJ</kbd> button to export the created UI to Python file (**\*.py**).
...
<table border="1">
  ...
  <tr>
    ...
    <td rowspan="1" colspan="1">
      dlg.on\_button\_clicked(<b>name="update\_picture"</b>, <b>callfunc=change\_bolt\_pic</b>)
    </td>
  </tr>

  <tr>
    <td rowspan="2" colspan="1">
      on\_execute\_button\_clicked
    </td>
    ...
    <td rowspan="1" colspan="1">
      dlg.on\_button\_clicked(<b>name="ButtonApply"</b>, <b>callfunc=on\_execute\_button\_clicked</b>)
    </td>
  </tr>
  ...
  <tr>
    ...
    <td rowspan="1" colspan="1">
      dlg.on\_button\_clicked(<b>name="ButtonOk"</b>, <b>callfunc=on\_execute\_button\_clicked</b>)
    </td>
  </tr>
</table>
