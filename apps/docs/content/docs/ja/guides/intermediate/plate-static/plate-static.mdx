...
1. Copy all the content in Sample3.zip under "PSJ Commands folder" where %userprofile%\AppData\Roaming\TechnoStar\JPT
   _Version_\PSJ_Commands\ by default. Place the files so that the hierarchy is set as follows:  
