...
export default function ShowcaseClient({ locale }: { locale: string }) {
    const tSearch = useTranslations('showcase.search');
    const tFilters = useTranslations('showcase.filters');
    const tResults = useTranslations('showcase.results');

    const solutions = useMemo(() => getSolutions(locale), [locale]);
    ...
    const [sortBy, setSortBy] = useState<string>(tFilters('sortByRelevant'));
    ...
    const filtered = useMemo(() => {
        ...
        // Sorting
        if (sortBy === tFilters('sortByCode')) {
            result.sort((a, b) => a.code.localeCompare(b.code));
        } else if (sortBy === tFilters('sortByComplexity')) {
        ...
        return result;
    }, [activeCategory, activeIndustry, search, sortBy, solutions, industries, tFilters]);
                ...
                <div className="flex flex-col gap-6 mb-10">
                    ...
                    <div className="flex flex-wrap items-center gap-4">
                        ...
                        <FilterDropdown
                            ...
                            icon={Building2}
                            prefix={tFilters('industryPrefix')}
                        />

                        {/* Sort */}
                        <FilterDropdown
                            value={sortBy}
                            options={[tFilters('sortByRelevant'), tFilters('sortByCode'), tFilters('sortByComplexity')]}
                            onChange={setSortBy}
                            prefix={tFilters('sortPrefix')}
                        />
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                        <div className="text-[10px] uppercase tracking-[0.15em] font-bold text-(--psj-text-3)">
                            {tResults('showing', { count: filtered.length, total: solutions.length })}
                        </div>
                        ...
                        {filtered.length === 0 ? (
                            ...
                            <div className="space-y-3">
                                {filtered.map((sol, i) => (
                                    ...
                                    >
                                        <SolutionCard sol={sol} onSelect={setSelectedSolution} />
                                    </motion.div>
                                ))}
                            </div>
                        )}
                    ...
                    <>
                        ...
                        >
                            <div className="p-6">
                                ...
                                <h3 className="text-lg font-bold mb-4">{tFilters('filterSolutions')}</h3>
                                {(Object.keys(categoryConfig) as Array<keyof typeof categoryConfig>).map((cat) => (
                                    <button
                                        key={cat}
                                        onClick={() => {
...
function SolutionCard({
    ...
    const tCard = useTranslations('showcase.card');
            ...
            <div className="grid md:grid-cols-12">
                ...
                >
                    <div className="space-y-4">
                        <MetaItem label={tCard('duration')} value={sol.duration} icon={Clock} />
                        <div>
                            <div
                                className="text-[10px] uppercase tracking-widest font-bold mb-1"
                                style={{ color: 'var(--psj-text-3)' }}
                    ...
                    <div className="space-y-2 mt-4">
                        <button onClick={() => onSelect(sol)} className="psj-btn-primary w-full justify-center">
                            {tCard('details')} <ArrowRight size={12} />
                        </button>
                    </div>
                </div>
            </div>
                            ...
                            <div className="space-y-5">
                                <div>
                                    <h4
                                        ...
                                        style={{ color: 'var(--psj-text-3)' }}
                                    >
                                        <Settings size={12} /> {tCard('capabilities')}
                                    </h4>
                                ...
                                <div>
                                    <h4
                                        ...
                                        style={{ color: 'var(--psj-text-3)' }}
                                    >
                                        <Box size={12} /> {tCard('deliverables')}
                                    </h4>
                                ...
                                <div
                                    ...
                                    style={{ borderTop: '1px solid var(--psj-border)' }}
                                >
                                    <MetaItem label={tCard('duration')} value={sol.duration} />
                                    <MetaItem label={tCard('complexity')} value={sol.complexity} />
                                </div>
                                <a href="#" className="psj-btn-primary w-full justify-center">
                                    {tCard('requestConsultation')}
                                </a>
                            </div>
                        </div>
                    </div>
