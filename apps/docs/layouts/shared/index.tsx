...
export type TransformFn = (option: LayoutTab, node: PageTree.Folder) => LayoutTab | null;

export interface GetLayoutTabsOptions {
    transform?: TransformFn;
}

const defaultTransform: GetLayoutTabsOptions['transform'] = (option, _node) => {
    return option;
};

export function getFirstUrl(folder: PageTree.Folder): string | undefined {
    if (folder.index) return folder.index.url;
    for (const child of folder.children) {
        if (child.type === 'page' && !child.external) return child.url;
        if (child.type === 'folder') {
            const url = getFirstUrl(child);
            if (url) return url;
        }
    }
    return undefined;
...
export function isLayoutTabActive(tab: LayoutTab, pathname: string) {
    if (tab.$folder) {
        return (
            PageTree.findPath(tab.$folder.children, (node) => node.type === 'page' && isActive(node.url, pathname)) !==
            null
        );
    }
