...
# Version: 5.0.1
# Docs:    /docs/cli/5.0.1/psj-gui/dlg-get_cell_value
# ---
from pyjdg import *
...
def get_table_cell_value(dlg,name,cell):
    ...
    if cellvector.size()>0:
        value_cell=dlg.get_cell_value(name="Table1",  # [hl:start]
            ...
            cell_column_id=cellvector[0].col_number)  # [hl:end]
        dlg.set_item_text(name="Textbox4",text=value_cell)
        value_cell=dlg.get_cell_value(name="Table1",  # [hl:start]
            cell_row_id=cellvector[0].row_number,
            cell_column_id=cellvector[0].col_number,option=1)  # [hl:end]
        dlg.set_item_text(name="Textbox5",text=value_cell)
        value_cell=dlg.get_cell_value(name="Table1",  # [hl:start]
            cell_row_id=cellvector[0].row_number,
            cell_column_id=cellvector[0].col_number,option=2)  # [hl:end]
...
def main():
    ...
    dlg.set_cell_value(name="Table1",
        cell_row_id=0,cell_column_id=0,value=["Option1","Option2","Option3"])
