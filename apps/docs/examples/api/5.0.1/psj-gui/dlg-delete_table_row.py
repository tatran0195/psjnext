...
# Desc:    Delete row at a specific position in the Table
# Version: 5.0.1
# Docs:    /docs/cli/5.0.1/psj-gui/dlg-delete_table_row
# ---
from pyjdg import *

def on_menu(dlg,name,menu):
    sel_ranges=dlg.get_table_sel_range(name="Table1")
    position=int(dlg.get_total_row(name="Table1"))-1
    if sel_ranges.size()>0:
        position=sel_ranges[0].top
    dlg.delete_table_row(name="Table1",position=position)  # [hl]
